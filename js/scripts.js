 ...
 *Add in th ebaility to removes a list item
 ...
 */
 ...
 function createListElement() {
     ...
     //Add it to our ul
     ul.appendChild(li);
     //Clear out the text unput after we create our new list item
     input.value = "";


     //Funcition to handle showing a list item as complete
     ...
     //Funxtion will be called whenever the delete button is clicked
 ...
 function createListItemSubmitButtom() {
     //Check to make sure our input has some text. An empty string will have a length of 0
     if (input.value.length > 0) {
    createListElement();
     }
 }

 //This is going to be used to create a list element when they enter key is pressed
 function createListItemEnterKey(event) {
     console.log(event.keyCode);

     //Create a new list item if there is text in the unput field and the enter key is pressed
 ...
 //Make it so when the submit button is clicked, the createListElement function is called
 submitButton.addEventListener("click", createListItemSubmitButtom);
